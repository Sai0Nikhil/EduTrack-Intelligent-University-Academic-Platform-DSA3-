...
\begin{itemize}
    ...
    \item \textbf{Option 7 (Crypto \& Security):} Tests 64-bit prime candidates using Miller-Rabin.
    \item \textbf{Option 8 (Comprehensive Verification Suite):} Executes the complete automated test harness validating all 16 algorithmic modules against regression assertions.
...
The graphical application provides a modern desktop interface built with Java Swing. Configured with a clean, high-contrast palette (Primary Blue \texttt{\#2563EB}, Dark Slate \texttt{\#0F172A}, Emerald Green \texttt{\#10B981}, and Off-White \texttt{\#F8FAFC}), the interface organizes functionality into 8 tabbed panels:
\begin{enumerate}
    ...
    \item \textbf{Tab 4: Resource Allocation (Network Flow):} Visual graph representation of flow networks, source/sink capacities, Dinic's layer levels, and minimum cut allocations.
    \item \textbf{Tab 5: Exam Timetabling (NP-Completeness):} Conflict graph inputs, DPLL SAT assignment viewer, and 2-approximation proctor cover metrics.
    ...
    \item \textbf{Tab 8: System Verification \& Benchmarks:} One-click execution of the 16-algorithm test harness with live status badges (\texttt{[PASS]}), assertion outputs, and microsecond latencies.
\end{enumerate}

To ensure complete interface responsiveness, all algorithmic workloads are dispatched asynchronously using background execution threads, preventing UI freeze during heavy parallel scans or flow computations.
